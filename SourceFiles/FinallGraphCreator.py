...
def GraphCreator(data):
    ...
    draw.text((25, 177), str(BlueGold), fill='black', font=fontSmall, align='center')
    ...
    draw.text((text_x_position, 177), str(RedGold), fill='black', font=fontSmall, align='center')


    # Drawing TeamKDA Text
    draw.text((150, 240), "Team KDA", fill='white', font=fontSmall, align='center')
    draw.text((25, 240), str(BlueKDA), fill='white', font=fontSmall, align='center')

    text_bbox = draw.textbbox((0, 0), str(RedGold), font=font)
    text_width = text_bbox[2] - text_bbox[0]  # Szerokość tekstu
    text_x_position = 485 - text_width  # Dopasowanie do prawej

    draw.text((text_x_position, 240), str(RedKDA), fill='white', font=fontSmall, align='center')
    ...
    #Paste EDU logo
    EDU = Image.open('./ImageToUse/EDU_LOGO.png').convert('RGBA')
    EDU = EDU.resize((169, 43))
    image.paste(EDU, (202, 545), EDU)

    #Paste PHE logo
    PHE = Image.open('./ImageToUse/PHE_LOGO.png').convert('RGBA')
    PHE = PHE.resize((50, 50))
    image.paste(PHE, (400, 545), PHE)
